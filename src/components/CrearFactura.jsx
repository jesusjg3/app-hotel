  ...
  const fetchDatosCompletos = async () => {
    ...
    try {
      ...
      if (dataReserva.cliente_id) {
        try {
          const resCliente = await fetch(`${API_URL}/clientes/${dataReserva.cliente_id}`, {
            headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
          });
          if (resCliente.ok) {
      ...
      try {
        const resServicios = await fetch(`${API_URL}/reservas/${reservaId}/servicios`, {
          headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
        });
        if (resServicios.ok) {
  ...
  const fetchRecursoInfo = async (reservaData, token) => {
    try {
      ...
      if (reservaData.tipo_reserva === "habitacion") {
        urlRecurso = `${API_URL}/habitaciones/${reservaData.id_objeto}`;
        precioCampo = "precio_noche";
      } else if (reservaData.tipo_reserva === "mesa") {
        urlRecurso = `${API_URL}/mesas/${reservaData.id_objeto}`;
        precioCampo = "precio_unitario";
      } else if (reservaData.tipo_reserva === "salon") {
        urlRecurso = `${API_URL}/salones/${reservaData.id_objeto}`;
